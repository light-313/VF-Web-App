...
class ProteinFeatureExtractor:
    ...
    def _load_model(self, model_name):
        ...
        self.model = UniRepModel.from_pretrained(model_name).to(self.device)
    ...
    def read_fasta(file_paths, max_seq_length=10000):
        """Reads FASTA files and filters sequences by length."""
                ...
                with open(path, "r") as f:
                    for record in SeqIO.parse(f, "fasta"):
                        ...
                        if len(seq) > max_seq_length:
                            print(f"Skipping sequence {record.id} due to length > {max_seq_length}")
                            continue
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(output_dir, f"train-{model_name}.h5")
